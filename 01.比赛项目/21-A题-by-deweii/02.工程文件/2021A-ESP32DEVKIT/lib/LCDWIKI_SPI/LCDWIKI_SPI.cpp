...
LCDWIKI_SPI::LCDWIKI_SPI(uint16_t model, int8_t cs, int8_t cd, int8_t miso, int8_t mosi, int8_t reset, int8_t clk, int8_t led)
{
	...
	MODEL = model;
	spicsPort = (uint8_t *)portOutputRegister(digitalPinToPort(_cs));
	...
	{
		spicdPort = (uint8_t *)portOutputRegister(digitalPinToPort(_cd));
	...
	{
		spimisoPort = (uint8_t *)portOutputRegister(digitalPinToPort(_miso));
		spimisoPinSet = digitalPinToBitMask(_miso);
		spimisoPinUnset = ~spimisoPinSet;
	}
	spimosiPort = (uint8_t *)portOutputRegister(digitalPinToPort(_mosi));
	spimosiPinSet = digitalPinToBitMask(_mosi);
	spimosiPinUnset = ~spimosiPinSet;

	spiclkPort = (uint8_t *)portOutputRegister(digitalPinToPort(_clk));
